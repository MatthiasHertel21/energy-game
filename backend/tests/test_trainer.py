...
from app.models import User, Cohort, Campaign, Scenario, Session, ActivityLog, CohortMember
...
def auth_headers(app):
    ...
    with app.app_context():
        ...
        token = create_access_token(identity=trainer.id)
...
def test_presence_endpoint_includes_session_info(app, client, auth_headers):
    ...
    with app.app_context():
        ...
        cohort = Cohort(name='Test Cohort')
        campaign = Campaign(name='Test Campaign', description='Test')
        scenario = Scenario(name='Test Scenario', campaign_id=None)  # Will set after commit
...
def test_presence_endpoint_filters_by_cohort(app, client, auth_headers):
    ...
    with app.app_context():
        ...
        activity1 = ActivityLog(user_id=student1.id, action='page_view', details={}, timestamp=datetime.utcnow())
        activity2 = ActivityLog(user_id=student2.id, action='page_view', details={}, timestamp=datetime.utcnow())

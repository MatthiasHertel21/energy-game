...
from app.models import Session, Device, Scenario, Round, Forecast, PlayerProgress
import json
    ...
    for rnd in rounds:
        ...
        if rnd.market_results:
            ...
            if 'hourly_results' in mr:
                hrs = mr['hourly_results']
                print(f"Hours cleared: {len(hrs)}")
                # Show hour 0
                if len(hrs) > 0:
                    h0 = hrs[0]
                    print(f"  Hour 0 SMP: {h0.get('smp_dam', 'N/A')}")
    
    # Get devices
    devices = Device.query.filter_by(session_id=377).all()
    print(f"\n=== DEVICES ({len(devices)}) ===")
    
    for d in devices:
        print(f"\n{d.subtype}: {d.capacity_mw} MW (ID: {d.id})")

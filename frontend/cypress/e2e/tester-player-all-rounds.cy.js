...
const REPORT_BASENAME = 'tester-ui-validation'
...
function buildValidationReport(seed) {
  return {
    ...
    total_rounds: seed?.total_rounds ?? null,
...
function runPlayerType(seed, player) {
  ...
  const totalRounds = Number(seed?.total_rounds || 6)
...
describe('Tester Player Screen All Rounds', () => {
  let seed

  before(() => {
    cy.readFile('cypress/fixtures/tester_ui_seed.json').then((data) => {
      seed = data
      validationState.runLabel = Cypress.env('TESTER_RUN_LABEL') || data?.tag || 'default'
      validationState.scenarioName = data?.scenario_name || 'Tester'
  ...
  it('runs all Tester player types through all rounds with real player-screen input and validates every KPI aggregation', () => {
    bumpCheck('existence')

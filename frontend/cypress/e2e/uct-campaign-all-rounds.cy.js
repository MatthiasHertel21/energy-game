...
import seed from '../fixtures/uct_campaign_seed.json'

const ROUND_TOLERANCE = 1.0
const VALUE_TOLERANCE = 0.05
const MWH_TOLERANCE = 0.001
const MIN_VALIDATED_VALUES_PER_PLAYER = 1000
const REPORT_DIR = 'cypress/results'
const REPORT_BASENAME = 'uct-campaign-validation'
...
function validateNumericLeaves(value, label) {
  if (typeof value === 'number') {
    assertFiniteValue(value, `${label} must be finite`)
    return
  }

  if (Array.isArray(value)) {
    value.forEach((entry, index) => validateNumericLeaves(entry, `${label}[${index}]`))
    return
  }

  if (value && typeof value === 'object') {
    Object.entries(value).forEach(([key, entry]) => validateNumericLeaves(entry, `${label}.${key}`))
  }
}

function getOrCreateScenarioStat(scenario) {
  const existing = validationState.scenarios.find((entry) => entry.scenario_id === scenario.scenario_id)
  if (existing) {
    return existing
  }
  ...
  validationState.scenarios.push(stat)
  return stat
}

function pushPlayerStat(scenarioStat, player) {
  const stat = {
  ...
  scenarioStat.players.push(stat)
  return stat
}
...
function buildValidationReport(seed) {
  return {
    ...
    campaign_name: validationState.campaignName || seed?.campaign_name || null,
    campaign_id: seed?.campaign_id ?? null,
    total_scenarios: seed?.total_scenarios ?? 0,
    total_players: seed?.total_players ?? 0,
    total_validated_values: validationState.totalChecks,
    minimum_required_validated_values: (seed?.total_players ?? 0) * MIN_VALIDATED_VALUES_PER_PLAYER,
...
function validateDeviceBreakdown(deviceBreakdown) {
  ...
  Object.entries(deviceBreakdown || {}).forEach(([deviceId, rows]) => {
    ...
    rows.forEach((row, rowIndex) => {
      ...
      const deviceType = String((row?.capacity_debug || {}).device_type || '').toLowerCase()
      const isBatteryRow = deviceType === 'battery'
        || Math.abs(toFloat(row?.battery_charged_mwh)) > 1e-9
        || Math.abs(toFloat(row?.battery_charge_cost_zar)) > 1e-9
        || Math.abs(toFloat(row?.battery_soc_start_pct)) > 1e-9
        || Math.abs(toFloat(row?.battery_soc_end_pct)) > 1e-9
      if (isBatteryRow) {
        if (firstBatterySoc === null) {
          firstBatterySoc = toFloat(row?.battery_soc_start_pct)
        }
        lastBatterySoc = toFloat(row?.battery_soc_end_pct)
...
function validateDeviceDetailAlignment(deviceBreakdown, details) {
  const balancing = details?.balancing || {}
  const co2 = details?.co2 || {}

  Object.entries(deviceBreakdown || {}).forEach(([deviceId, rows]) => {
    const balancingRows = Array.isArray(balancing?.[deviceId]) ? balancing[deviceId] : []
    const co2Rows = Array.isArray(co2?.[deviceId]) ? co2[deviceId] : []
    const balancingByHour = Object.fromEntries(balancingRows.map((row) => [row?.hour_idx, row]))
    const co2ByHour = Object.fromEntries(co2Rows.map((row) => [row?.hour_idx, row]))

    ;(rows || []).forEach((row) => {
      const hour = row?.hour
      const balancingRow = balancingByHour[hour]
      const co2Row = co2ByHour[hour]

      if (balancingRow) {
        if (balancingRow?.da_dispatched_mwh !== undefined) {
          assertClose(row?.da_dispatched_mwh, balancingRow?.da_dispatched_mwh, VALUE_TOLERANCE, `${deviceId} DA dispatched must align with balancing detail`) }
        if (balancingRow?.id_dispatched_mwh !== undefined) {
          assertClose(row?.id_dispatched_mwh, balancingRow?.id_dispatched_mwh, VALUE_TOLERANCE, `${deviceId} ID dispatched must align with balancing detail`) }
        if (balancingRow?.total_dispatched_mwh !== undefined) {
          assertClose(row?.total_dispatched_mwh ?? row?.dispatched_mw, balancingRow?.total_dispatched_mwh, VALUE_TOLERANCE, `${deviceId} total dispatched must align with balancing detail`) }
      }

      if (co2Row) {
        const detailCo2 = co2Row?.co2_kg ?? co2Row?.emissions_kg
        if (detailCo2 !== undefined) {
          assertClose(row?.co2_kg, detailCo2, VALUE_TOLERANCE, `${deviceId} CO2 must align with CO2 detail`) }
      }
    })
  })
}

function validateRanking(ranking, myResult) {
  bumpCheck('existence')
  expect(ranking, 'round ranking').to.be.an('array').and.have.length(1)
  const entry = ranking[0] || {}
  if (entry?.type !== undefined) {
    assertEqual(entry.type, myResult?.type, 'ranking type must match my_result type')
  }
  if (entry?.total_score !== undefined) {
    assertClose(entry.total_score, myResult?.total_score, VALUE_TOLERANCE, 'ranking total_score must match my_result total_score')
  }
  if (entry?.profit !== undefined) {
    assertClose(entry.profit, myResult?.profit, ROUND_TOLERANCE, 'ranking profit must match my_result profit')
  }
}
...
function validateKpis(kpis, myResult, weights) {
  ...
  validateDeviceDetailAlignment(deviceBreakdown, myResult?.device_hourly_details)
...
function selectEditableDevice(player) {
  const preferredId = player?.primary_device_id
  if (preferredId) {
...
function buildRoundSubmissionPayload(fullForecast, sessionId, roundNum, roundSpan) {
...
function submitRoundFromUi(roundNum, sessionId, auth, roundSpan) {
  const plan = getRoundPlan(roundNum, {})
    ...
    if ($body.find('[data-cy="submit-current-round"]').length === 0) {
      ...
      return cy.request({
        ...
        return cy.request({
          ...
          headers: authHeaders(auth.access_token),
          body: buildRoundSubmissionPayload(body, sessionId, roundNum, roundSpan),
...
function validateRoundPayload(roundData, player, roundNum) {
  assertEqual(roundData?.round, roundNum, 'round result round must match requested round')
  ...
  validateRanking(roundData?.ranking, roundData?.my_result)
  validateNumericLeaves(roundData, `round_${roundNum}`)
...
function runPlayerScenario(seed, scenario, player, scenarioStat) {
  const roundSnapshots = []
  const totalRounds = Number(scenario?.total_rounds || 6)
  const roundSpan = Number(scenario?.round_span_hours || 6)
  const playerStat = pushPlayerStat(scenarioStat, player)

  cy.log(`Run ${scenario.scenario_name} / ${player.type_id}`)
  return cy.request('POST', '/api/auth/login', {
    ...
    return cy.request({
      ...
      headers,
      body: { scenario_id: scenario.scenario_id, campaign_id: seed.campaign_id },
    }).then(({ body }) => {
          ...
          return waitForSessionStatus(sessionId, auth.access_token, ['round_active', 'running']).then(() => {
            const runRound = (roundNum) => {
              return openPlayerScreen(sessionId, auth).then(() => {
                ...
                submitRoundFromUi(roundNum, sessionId, auth, roundSpan)
                ...
                return waitForSessionStatus(sessionId, auth.access_token, ['round_results', 'scenario_complete']).then(() => {
                  return cy.request({
                    ...
                    validateRoundPayload(roundData, player, roundNum)
                    ...
                    scenarioStat.totalChecks += roundChecks
                    ...
                    return waitForSessionStatus(sessionId, auth.access_token, ['scenario_complete', 'round_results']).then((statusBody) => {
                      ...
                      return cy.request({
                        ...
                        validateNumericLeaves(finalData, `final_${scenario.scenario_id}_${player.type_id}`)
                        ...
                        scenarioStat.totalChecks += finalChecks
                      })
                    })
                  })
                })
...
describe('UCT Campaign All Rounds', () => {
  before(() => {
    validationState.runLabel = Cypress.env('UCT_RUN_LABEL') || seed?.tag || 'default'
    validationState.campaignName = seed?.campaign_name || 'UCT Campaign'
  ...
  if (!Array.isArray(seed?.scenarios) || seed.scenarios.length === 0) {
    it('requires a generated UCT seed fixture before the validation run', () => {
      throw new Error('Missing seeded UCT fixture. Run frontend/scripts/run-uct-campaign-validation.sh first.')
    })
    return
  }
  ...
  seed.scenarios.forEach((scenario) => {
    ;(scenario?.players || []).forEach((player) => {
      it(`runs ${scenario.scenario_name} / ${player.type_id} through all rounds`, () => {
        const scenarioStat = getOrCreateScenarioStat(scenario)
        return runPlayerScenario(seed, scenario, player, scenarioStat).then(() => {
          return cy.writeFile(`${REPORT_DIR}/${REPORT_BASENAME}-progress-${validationState.runLabel}.json`, buildValidationReport(seed), { log: false })
        })
      })
    })
  })

  it('writes the cumulative UCT validation report', () => {
    bumpCheck('existence')
    expect(seed?.scenarios, 'seed scenarios').to.have.length.greaterThan(0)
    const minimumValidatedValues = (seed?.total_players || 0) * MIN_VALIDATED_VALUES_PER_PLAYER
    bumpCheck('bounds')
    expect(validationState.totalChecks, 'validated business and arithmetic values').to.be.greaterThan(minimumValidatedValues)
